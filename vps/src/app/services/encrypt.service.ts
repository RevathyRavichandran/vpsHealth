...
export class EncryptService {
    ...
    encrypt(dataToEncrypt, aesPublicKey) {
        ...
        let key = CryptoJS.codec.hex.fromBits(
            CryptoJS.random.randomWords(32, 0)
        );
        p = {
            adata: '',
            iter: 1000,
    ...
    decrypt(randKey, timeStamp, responseHash, encryptedRes, aesPublicKey) {
        let decryptedKey = this.decryptAES(
            this.decode(randKey),
            aesPublicKey,
            timeStamp
        );
        let decryptedResponse = this.decryptAES(
            this.decode(encryptedRes),
            decryptedKey,
            timeStamp
        );
    ...
    decryptAES(encryptedData, key, timestamp) {
        ...
        try {
            ...
            plaintext = CryptoJS.decrypt(
                key,
                JSON.stringify(dataToDecrypt),
                {},
                rp
            );
        } catch (e) {
